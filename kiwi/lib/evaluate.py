...
def evaluate_from_options(options):
    ...
    if pipeline_options.pred_target:
        for pred_file in pipeline_options.pred_target:
            ...
            if is_wmt18_pred_format:
                ...
                pred_files[const.GAP_TAGS].append(
                    (str(pred_file), pred_gaps)
                )
            pred_files[const.TARGET_TAGS].append(
    ...
    if pipeline_options.pred_source:
        for pred_file in pipeline_options.pred_source:
            ...
            pred_files[const.SOURCE_TAGS].append(
    ...
    threshold = None
    if pipeline_options.pred_cal and pipeline_options.gold_cal and \
            pipeline_options.type == 'probs':
    ...
    if const.SENTENCE_SCORES in golds:
        ...
        sents_avg = pipeline_options.sents_avg if \
            pipeline_options.sents_avg else pipeline_options.type
